//Phase unwrapper described in:
//"Fast three-dimensional phase-unwrapping algorithm based on sorting by 
//reliability following a noncontinuous path"
//by Hussein Abdul-Rahman, Munther A. Gdeisat, David R. Burton, and Michael J. Lalor, 
//Applied Optics, Vol. 46, No. 26, pp. 6623-6635, 2007
//This program was written on 29th August 2007
//The wrapped phase volume is floating point data type
//Also, the unwrapped phase volume is floating point read the data from 
...
struct EDGE
{    
	float reliab;			//edge reliabilty - it depends on the two voxels
	VOXELM *pointer_1;		//pointer to the first voxel
    VOXELM *pointer_2;		//pointer to the second voxel
    int increment;			//No. of 2*pi steps to add to one of the voxels to unwrap it with respect to the second 
}; 

void read_data(char *inputfile,float *Data, int length)
{
	printf("Load wrapped image\n");
	FILE *ifptr;
...
void write_data2(char *outputfile,float *Data,int length)
{
	printf("Save second difference map\n");
	FILE *ifptr;
	ifptr = fopen(outputfile,"wb");
	if(ifptr == NULL) printf("Error opening the file\n");
	fwrite(Data,sizeof(float),length,ifptr);
...
//another version of Mixtogether but this function should only be used with the sort program
...
//this might be the fastest sort program; 
//see explanation in quickSort function below
...
//we need to sort edge array. Each edge element consists of 16 bytes.
//In the normal sort program we compare two elements in the array and exchange
...
//element. The edge needs 4 bytes which makes the sorting operation faster.
//After sorting all indices, we will know the position for each index. 
...
//--------------------start initialise voxels ----------------------------------
//initialse voxels. See explanation of voxel class above.
//initially every voxel is a gorup by itself
...
void extend_mask(unsigned char *input_mask, unsigned char *extended_mask, int volume_width, int volume_height, int volume_depth)
{
	...
	//extend mask for the volume except borders
...
//calculate reliability of volume's horizontal edges
//calculated by adding voxel reliability to that of its right neighbour
//edge calculated between a voxel and its next neighbour
...
void  gatherVOXELs(EDGE *edge)
{
	...
	for (k = 0; k < No_of_edges; k++)
	{
		...
		//initially each voxel is a group by itself and one voxel can construct a group
		...
		if (VOXEL2->head != VOXEL1->head)
		{
			...
			else
            {
				group1 = VOXEL1->head;
                group2 = VOXEL2->head;
				//the no. of voxels in VOXELM 1 group is larger than the no. of voxels
...
{  
	...
	//sort EDGEs according to reliability. The VOXELs with higher reliability (small value) first
	//run only one of the two functions (quick_sort() or quicker_sort() )
	//if run stuck because of quicker_sort(), use quick_sort()
	...
	write_data(argv[3], UnwrappedVolume, volume_size);
        write_data2(argv[7], UnwrappedVolume1, volume_size);
