function D=m1_tl_medin_dipole_kernel(varargin)
% Generation of the Dipole Kernel
%   D = m1_tl_medin_dipole_kernel(matrix_size, voxel_size, B0_dir, ...)
%
%   output
%   D - m1_tl_medin_dipole kernel saved in Fourier space
% 
%   input
...
%   Modified by Tian Liu on 2013.07.22
%   Last modified by Julio Acota-Cabronero, 09/05/2019
...
% Take the absolute to B0_dir if it contains negative values
abs_B0_dir = abs(B0_dir);

if sum(B0_dir-abs_B0_dir)~=0
    B0_dir = abs_B0_dir;
    disp(['WARNING: Negative B0_dir components found - calculate dipole kernel for B0_dir = [' num2str(B0_dir) ']'])
end
