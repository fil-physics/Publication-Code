function [rmse,hfen,ssim] = m1_simil(imaroot,refroot,mskroot,saveSSIMmap)
% Calculate similarity metrics across 'mskroot' between two images with 
% fileroots 'imaroot' and 'refroot'
%
% 'imaroot'.nii(.gz) can be a 4D NIFTI file
% 
% SINTAX
%  [rmse,hfen,ssim] = m1_simil(imaroot,refroot,mskroot,saveSSIMmap)
%
% Created by Julio Acosta-Cabronero, 15 Nov 2017

[ima,st]    = m1_nifti_load(imaroot);
[ref]       = m1_nifti_load(refroot);
[msk]       = m1_nifti_load(mskroot);

ref = ref.*msk;
tttmap=[];

for ccc = 1:(size(ima,4))
    ima_ttt             = ima(:,:,:,ccc).*msk;
    rmse(ccc)           = compute_rmse(ima_ttt,ref);
    hfen(ccc)           = compute_hfen(ima_ttt,ref);
    [ssim(ccc),tttmap]  = compute_ssim(ima_ttt,ref);
    ssim_map(:,:,:,ccc) = tttmap;
end

save('simil_rmse.txt','rmse','-ascii')
save('simil_hfen.txt','hfen','-ascii')
save('simil_ssim.txt','ssim','-ascii')
...
if saveSSIMmap
    m1_nifti_save(ssim_map,st,'simil_ssim_map')
end
    
% save('ssim_map.mat','ssim_map')
