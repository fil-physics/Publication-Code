%% Correlation ratio (CR) vs Mutual information: MT-3DEPI data in MP2RAGE space 
...
for sub= SUBJT(~ismember(SUBJT,[4,7])) %% subjects 4 and 7 were excluded due to extensive motion [4,7]
    ...
    string_ats1= 'vmPFC_highres.nii.gz';
    string_ats2= 'dmPFC_highres.nii.gz';
    ...
    string1 = 'initial_highres2highres.nii'; %% MT3DEPI data in MP2RAGE space(no correction)
    a0m=dir([dir_reg_nc,string1]);
    nf=load_untouch_nii([dir_reg_nc,a0m(1).name]);
    vol_nc = double(nf.img);

    string1 = 'initial_highres2highres_jac.nii';%% MT3DEPI data in MP2RAGE space (reversed-PE) 
    a0m=dir([dir_reg_bp,string1]);
    nf=load_untouch_nii([dir_reg_bp,a0m(1).name]);
    vol_bp = double(nf.img);

    string1 = 'initial_highres2highres.nii';%% %% MT3DEPI data in MP2RAGE space(B0 field mapping) 
