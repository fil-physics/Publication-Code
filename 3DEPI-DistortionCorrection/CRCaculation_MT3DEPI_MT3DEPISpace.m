%% Correlation ratio (CR) calculation of distorted MP2RAGE data in  MT-3DEPI space 
...
for sub= SUBJT(~ismember(SUBJT,[4,7])) %% subjects 4 and 7 were excluded due to extensive motion [4,7]
    ...
    string_ats=  'oxf_thr0_2initialhighres.nii.gz';  %% atlas in MT-3DEPI space for parcellation
    a0m=dir([dir_ats,string_ats]);
    nf=load_untouch_nii([dir_ats,a0m(1).name]);
    ats = (nf.img);
    regions= max(max(max(ats)));

    string1 = 'clean_uni_reo_brain2EPI.nii.gz'; %% distorted MP2RAGE data in MT-3DEPI space using no correction
    a0m=dir([dir_reg_nc,string1]);
    nf=load_untouch_nii([dir_reg_nc,a0m(1).name]);
    vol_nc = double(nf.img);

    string1 = 'clean_uni_reo_brain2EPI_rev.nii.gz'; %% distorted MP2RAGE data in MT-3DEPI space using reversed-PE
    %string1 = 'clean_uni_reo_brain2EPI.nii.gz';  
    a0m=dir([dir_reg_bp,string1]);
    nf=load_untouch_nii([dir_reg_bp,a0m(1).name]);
    vol_bp = double(nf.img);

    string1 = 'clean_uni_reo_brain2EPI.nii.gz'; %% distorted MP2RAGE data in MT-3DEPI space using B0 field mapping
    ...
    string1 = 'AP.nii';  %% MT-3DEPI data as a reference
    a0m=dir([dir_seg_str,string1]);
    nf=load_untouch_nii([dir_seg_str,a0m(1).name]);
...
%%Group-level GM edge extraction for CR improvement map
...
lbtmean1=sum(lbt1,4);
string_magn = 'WB_NTbp_th0_oxf_native.nii.gz';
save_nii(make_nii(single(lbtmean1)),string_magn)
nf1=load_untouch_nii('WB_NTbp_th0_oxf_native.nii.gz');
nf1.hdr.hist = nf.hdr.hist;
save_untouch_nii(nf1,string_magn)

lbtmean2=sum(lbt2,4);
string_magn = 'WB_NTb0_th0_oxf_native.nii.gz';
save_nii(make_nii(single(lbtmean2)),string_magn)
nf1=load_untouch_nii('WB_NTb0_th0_oxf_native.nii.gz');
nf1.hdr.hist = nf.hdr.hist;
save_untouch_nii(nf1,string_magn)
